...
public final class Files_Utils_DG {
    ...
    public static String FilesUpload_transferTo_spring(HttpServletRequest request, MultipartFile multipartFile, String filePath) {
        if (multipartFile != null) {
            ...
            String absolutePath = getAndSetAbsolutePath(request, filePath,uuid, suffix);
            //return relative Path
            String relativePath = getRelativePath(filePath,uuid,suffix);
            try {
                //save file
    ...
    public static String getUUIDName(String uuid,String suffix) {
        return uuid + suffix;// make new file name
    }

    //return server absolute path（real path） the style is  “server absolute path/DataPath/UUIDName”filePath example "/files/Upload"
    public static String getAndSetAbsolutePath(HttpServletRequest request, String filePath,String uuid,String suffix) {
        ...
        return savePath + getUUIDName(uuid,suffix);
    }

    //get the relative path of files style is “/filePath/DataPath/UUIDName”filePath example "/files/Upload"
    public static String getRelativePath(String filePath, String uuid,String suffix) {
        return filePath + File.separator + getDataPath() + File.separator + getUUIDName(uuid,suffix);//example:/files/Upload/20160912/
    }
}